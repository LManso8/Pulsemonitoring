...
* **Peak Vector Sum - PVS (mm/s):** The True Vector Sum computed from the instantaneous tri-orthogonal coordinates ($V = \sqrt{V_x^2 + V_y^2 + V_z^2}$).

## Implemented Frameworks
The module programmatically implements the boundary curves of the following standards:
...
4. **Occupational & Environmental Control:**
   * *Static Thresholds:* Continuous background vibrations (0.15 mm/s) and transient/impulsive detonations (0.30 mm/s).

## Outputs
For each processed event, `standards_check.py` evaluates the data and returns:
1. **Dynamic Threshold Limit:** The exact allowable velocity limit (in mm/s) calculated dynamically based on the input frequency and selected structural class.
